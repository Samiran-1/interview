...
export const ScoreBadge = ({ score = 0, className = "" }: ScoreBadgeProps) => {
  const numScore = typeof score === "number" ? score : 0;
  const getScoreColor = (s: number) => {
    if (s >= 90) return "bg-emerald-500/10 text-emerald-400 border-emerald-500/20";
    if (s >= 75) return "bg-blue-500/10 text-blue-400 border-blue-500/20";
    if (s > 0) return "bg-amber-500/10 text-amber-400 border-amber-500/20";
